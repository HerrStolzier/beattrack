...
function similarityLabel(score: number): string {
  if (score >= 0.9) return "Sehr nah dran";
  if (score >= 0.7) return "Passender Fund";
  return "Weiter draußen";
}
      ...
      <div className="p-4 sm:p-5">
        ...
        <div className="flex items-start justify-between gap-3">
              ...
              <div className="mt-1.5 flex items-center gap-2">
                {song.bpm != null && (
                  <span className="inline-flex items-center gap-1 rounded-md bg-amber/8 px-2 py-0.5 font-mono tabular-nums text-[11px] font-medium text-amber-light">
                    {Math.round(song.bpm)}
                    <span className="text-[9px] font-normal text-amber-light/50">
                      BPM
                    </span>
                  </span>
                )}
          ...
          >
            ...
            <p
              className={`text-[10px] font-medium tracking-widest uppercase ${pctColor} opacity-70`}
            >
        ...
        >
          <motion.div
            ...
            style={{
              boxShadow: isTop3 ? "0 0 12px var(--color-amber-glow)" : "none",
            }}
            initial={{ width: 0 }}
            animate={{ width: `${pct}%` }}
            transition={{
        ...
        <div className="mt-3 flex items-stretch gap-1.5">
          ...
          {hasDeezer ? (
            ...
            >
              {isExpanded ? (
                ...
                <svg
                  className="h-5 w-5 transition-transform duration-200 group-hover/play:scale-110"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
          ...
          <button
            onClick={() => {
              ...
              window.open(
                searchUrl("spotify", song.artist, song.title),
                "_blank",
              );
            }}
          ...
          <button
            onClick={() => {
              ...
              window.open(
                searchUrl("youtube", song.artist, song.title),
                "_blank",
              );
            }}
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h2 className="min-w-0 truncate text-sm font-semibold text-text-secondary">
          Passt zu <span className="text-text-primary">{querySong.title}</span>
        ...
        <button
          ...
          title={
            djMode
              ? "DJ-Modus aus"
              : "DJ-Modus an — zeigt harmonische Kompatibilität"
          }
        >
          <svg
      ...
      {results.length === 0 && (
        <div className="rounded-xl bg-surface-raised p-6 text-center">
          ...
          <p className="mt-1 text-[11px] text-text-tertiary">
            Der Katalog hat gerade keinen guten Klang-Nachbarn für diesen Track.
            Ein anderer Seed kann bessere Funde liefern.
          </p>
        </div>
      )}
